...
void Srf05_MidFront(void)
{
	...
	Srf05_Struct.Srf05_GPIO_Trigger		= GPIOC;
	Srf05_Struct.Srf05_Trigger_Pin		= GPIO_Pin_0;
...
void Srf05_LeftFront(void)
{
	Srf05_Struct.Srf05_ICTIM					= TIM2;
	Srf05_Struct.Srf05_GPIO_Trigger		= GPIOC;
	Srf05_Struct.Srf05_Trigger_Pin		= GPIO_Pin_1;
...
void Srf05_RightFront(void)
{
	Srf05_Struct.Srf05_ICTIM					= TIM2;
	Srf05_Struct.Srf05_GPIO_Trigger		= GPIOC;
	Srf05_Struct.Srf05_Trigger_Pin		= GPIO_Pin_4;
	Srf05_Struct.Srf05_GPIO_Echo			= GPIOB;
...
void Srf05_BackEnd(void)
{
	Srf05_Struct.Srf05_ICTIM					= TIM2;
	Srf05_Struct.Srf05_GPIO_Trigger		= GPIOC;
	Srf05_Struct.Srf05_Trigger_Pin		= GPIO_Pin_5;
	Srf05_Struct.Srf05_GPIO_Echo			= GPIOB;
...
void Peripheral_Config(void)
{
	...
	USART2_Config(115200); 			//GPS USART first priority 
	USART6_Config(115200); 			//Sending and controling USART1
	Encoder_Config();				
	Srf05_MidFront();
	Srf05_LeftFront();
	Srf05_RightFront();
...
int main(void)
{
	...
	while(1)
	{
		...
		/*-----------------------------------------------------------------*/
		/*-----------------------------------------------------------------*/
		/*-------- Sensor Srf05 section -----------------------------------*/
		if(Status_CheckStatus(&VehStt.Srf05_TimeOut_Flag))
		{
			switch(Veh.Srf05_Selected_Sensor)
			{
				case Sensor_Mid:
					if(Mid.GetData_Flag)
					{
					...
					Veh.Srf05_Selected_Sensor++;
					break;
				
				case Sensor_Left:
					if(Left.GetData_Flag)
					{
						Srf05_ClearFlag(&Left);
					}
					else
					{
							Left.Distance = 100;
				...
				case Sensor_Right:
					if(Right.GetData_Flag)
					{
						Srf05_ClearFlag(&Right);
					}
					else
					{
							Right.Distance = 100;
				...
				case Sensor_Back:
					if(Back.GetData_Flag)
					{
						Srf05_ClearFlag(&Back);
					}
					else
					{
							Mid.Distance = 100;
			...
			if(Veh.Srf05_Selected_Sensor > 4)
			{
				Veh.Srf05_Selected_Sensor = 1;
			}
			
			if(Veh.Srf05_Selected_Sensor == Sensor_Mid)
				Srf05_StartSensorMid();
			else if(Veh.Srf05_Selected_Sensor == Sensor_Left)
				Srf05_StartSensorLeft();
			else if(Veh.Srf05_Selected_Sensor == Sensor_Right)
				Srf05_StartSensorRight();
			else if(Veh.Srf05_Selected_Sensor == Sensor_Back)
				Srf05_StartSensorBack();
